...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.book3-brochure .p'+page);
		img.css({width: container.width(), height: container.height()});
		img.appendTo($('.book3-brochure .p'+page));
...
function updateTabs() {
	...
	var tabs = {7: 'Page 7', 12:'Page 12', 14:'Page 14', 16:'Page 16', 23:'Page 23'},
		left = [],
		right = [],
		book = $('.book3-brochure'),
	...
	$('.book3-brochure .tabs .left').html(left.join(''));
	$('.book3-brochure .tabs .right').html(right.join(''));
	...
	flipbook.turn({
		...
		turned: function(e, page, view) {
			...
			if (page!=1 && page!=book.turn('pages'))
				$('.book3-brochure .tabs').fadeIn(500);
...
bookshelf.loadSample(sampleName, function(action) {
	...
	if (!sample.flipbook) {
		...
		var bookClass = (Modernizr.csstransforms) ?
			'book3-transform book3-brochure' :
			'book3-brochure';
