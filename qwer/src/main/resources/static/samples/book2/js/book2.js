...
function updateDepth(book, newPage) {
	...
	if (newPage>3)
		$('.book2-book .p2 .depth').css({
			width: depthWidth,
			left: 20 - depthWidth
		});
	else
		$('.book2-book .p2 .depth').css({width: 0});

		depthWidth = Math.round(16*Math.min(1, (pages-page)*2/pages));

	if (newPage<pages-3)
		$('.book2-book .p111 .depth').css({
			width: depthWidth,
			right: 20 - depthWidth
		});
	else
		$('.book2-book .p111 .depth').css({width: 0});
...
function loadPage(page) {
	...
	if (page==3)
		$.ajax({url: samplePath + 'pages/page' + page + '.html?'+Math.random()}).
				...
				$('.book2-book .p' + page).html(pageHtml);
			});
...
function loadFlipbook(flipbook) {
	...
	flipbook.turn({
		...
		turning: function(e, page, view) {
			...
			if (page>=2)
				$('.book2-book .p2').addClass('fixed');
			else
				$('.book2-book .p2').removeClass('fixed');

			if (page<book.turn('pages'))
				$('.book2-book .p111').addClass('fixed');
			else
				$('.book2-book .p111').removeClass('fixed');
...
bookshelf.loadSample(sampleName, function(action) {
	...
	if (!sample.flipbook) {
		...
		var bookClass = (Modernizr.csstransforms) ?
			'book2-book-transform book2-book' :
			'book2-book';
